...
is 100% null throughout the file's history, and ZIP is truncated to 3 digits, so no
...
and the joint-application-only `*_joint` fields. All are 97.8-100% null within the
population.
...
| Rollout + structural, stacked (6 cols) | 7.9-17.0% | Falls post-2012, never reaches 0%: a structural residual persists | Sentinel 999; one column also gets its own flag |
...
republication as-is. No new consent for research use was obtained. Use relies on the
loans having originally been part of a public release by Lending Club and on the CC0
terms of the Kaggle republication (§8). No subcontractors were involved on this project's
side.

## 6. Preprocessing, Cleaning, and Labeling

The 151 raw columns were classified by a single criterion against the official
dictionary: whether each was known at origination or only born/updated afterward. That
produced 3 identifiers (dropped), 37 post-origination columns (dropped as leakage,
confirmed via univariate AUC: `recoveries` and `collection_recovery_fee` nearly
perfect-predict the outcome by construction), 99 origination-time candidates, 4 free-text
...
is re-derivable from it via notebooks 01-05 and `src/`, and nothing is hand-edited.
...
**Intended use**: educational/portfolio demonstration of credit-risk modeling
methodology, covering temporal validation, profit-based evaluation, documented
missing-data handling, and honest reporting of model limitations.
...
degrades severely under a model trained on 36-month loans (AUC drop of 0.04-0.07, and
M1's profit gain turns negative). The two terms are not interchangeable risk pools.
...
process exists beyond commit history, and there is no planned update schedule. This is a
static, one-time analysis, not a maintained data feed.
