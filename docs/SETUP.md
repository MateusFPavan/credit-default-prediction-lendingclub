...
1. Checks that `data/raw/accepted_2007_to_2018Q4.csv` exists. Aborts with a clear
   message (pointing back to §3) if it doesn't.
...
6. `src/verify_pipeline.py`: an independent, `src/`-only check that retrains the model
   and asserts the test-set profit reproduces **exactly**. It exits with an error if it
   doesn't.
...
≤2013, validation 2014, test 2015 (plus a 60-month transfer set). This is never a
random split. Re-splitting randomly would leak future information into training and
would not reproduce any number in this document or in `docs/FACTS.md`.
...
`src/verify_pipeline.py` asserts this exact figure (to the cent) in a single run. No
repeated sampling is required to reproduce it.
...
- **Test profit doesn't match exactly**: check that XGBoost is running with `n_jobs=1`
  ...
  Either change affects the result (§6).
