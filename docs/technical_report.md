...
delta over the $233.2M an approve-all policy already yields — the report leads with the
...
realized loss) vs. 0 (Fully Paid) — derived only from loans with a concluded outcome.
...
$2,023.62) — treating a false negative and a false positive as equally costly, as
accuracy implicitly does, misrepresents the actual economics of the decision.
...
imputation rule — the strongest methodological claim of the cleaning phase, since two of
the three mechanisms below have the null itself carry directional risk signal.
...
**Two engineered candidates were tried and dropped — reported here as a strength of the
process, not omitted:**
...
candidate configurations across **three expanding temporal windows** — train ≤2011 →
validate 2012; train ≤2012 → validate 2013; train ≤2013 → validate 2014 — optimizing
mean expected profit across all three windows, not AUC and not a single year.
...
At threshold 0.31: **10,644 loans rejected**; avoided loss $32.2M; forgone interest
$23.1M; net $9.0M (matches §7.1 exactly). False-negative cost is approximately **11.9x**
false-positive cost — consistent with the 2.67x median cost asymmetry (§2), amplified
because false negatives concentrate in larger, riskier loans. **The model rejects 9.2% of
eventual defaulters** — the $9.0M gain comes from identifying a small, financially
concentrated slice of the worst cases, not from bulk rejection (96.2% of applications are
approved).
...
the yield of a random 10% cut, and the plainest translation of the model into a policy
statement for a non-technical reader.
...
**The model is least reliable exactly in the highest-risk, lowest-income segment** —
precisely where a lender would most need precision, and the reverse of what the headline
AUC (0.6846) alone would suggest.
...
optimistic — a limitation that matters directly for any threshold-based decisioning built
on top of it, not just an academic footnote.

**SHAP** (50,000-row stratified test sample; a full-test run was estimated at ~22 minutes,
over the compute budget for this analysis). Top features by mean |SHAP|: `fico_range_low`,
`installment_to_income`, `annual_inc`, `acc_open_past_24mths`, `dti`. One confound is
worth naming: `verification_status` shows a Simpson's-paradox pattern — its univariate
association (verified income correlates with *higher* default) partly reverses once the
other 78 features are controlled for, indicating some of the univariate signal was
confounded by correlated features rather than a direct causal reading of verification
status itself.
...
legitimate temporal split, never touching validation or test). Reliability improved
...
- **Term non-transferability.** Not valid for 60-month loans without a dedicated
  scorecard (§9).
- **Calibration is optimistic, not conservative** (§8) — relevant to any downstream use
  of the raw probability, not just the fixed 0.31 threshold evaluated here.
...
`python run_all.py` reproduces the essential path — raw CSV → cleaned dataset → temporal
split → features → final model → verified test result — in ~3.1 minutes, and asserts the
$242,230,710.89 test profit reproduces exactly. The walk-forward tuning and bootstrap
experiments referenced in §6.2, §7.2, and §10 are not re-run by that entry point (they
take hours); they are preserved in the numbered working notebooks (`notebooks/06`–`11`)
and summarized in `docs/FACTS.md`. Full instructions: `docs/SETUP.md`.
