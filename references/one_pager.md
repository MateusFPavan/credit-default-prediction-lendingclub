...
optimized it to maximize portfolio profit, not prediction accuracy: for a lender, those
are not the same target.

## Headline Impact

**On the held-out 2015 test set, the model rejects just 3.8% of loan applications,
avoiding $32.2M in losses at a cost of $23.1M in foregone interest.** The result is a net
gain of $9.0M over an approve-everyone policy. That gain is statistically robust (95% CI
...
- **Net gain of +$9.0M** over approving every applicant. This was chosen by optimizing
  profit, not accuracy, and that choice mattered: the winning model ties a simpler
  baseline on AUC (0.68) but wins decisively on profit, value a standard accuracy contest
